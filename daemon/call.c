...
struct call_media *call_media_new(struct call *call) {
	...
	med->call = call;
	codec_store_init(&med->codecs_recv, med);
	codec_store_init(&med->codecs_send, med);
	return med;
}
		...
		struct stream_params *sp, struct sdp_ng_flags *flags)
{
	if (!flags || flags->opmode != OP_ANSWER) {
		...
		ilogs(codec, LOG_DEBUG, "Updating receiver side codecs for offerer " STR_FORMAT " #%u",
				...
				other_media->index);
		codec_store_populate(&other_media->codecs_recv, &sp->codecs, flags->codec_set);
		codec_store_strip(&other_media->codecs_recv, &flags->codec_strip, flags->codec_except);
		codec_store_offer(&other_media->codecs_recv, &flags->codec_offer, &sp->codecs);
		if (!other_media->codecs_recv.strip_full)
			codec_store_offer(&other_media->codecs_recv, &flags->codec_transcode, &sp->codecs);
		codec_store_accept(&other_media->codecs_recv, &flags->codec_accept, NULL);
		codec_store_accept(&other_media->codecs_recv, &flags->codec_consume, &sp->codecs);
		codec_store_track(&other_media->codecs_recv, &flags->codec_mask);

		// do the same for the sender side
		// XXX use a duplication routine here?
		ilogs(codec, LOG_DEBUG, "Updating sender side codecs for offerer " STR_FORMAT " #%u",
				STR_FMT(&other_media->monologue->tag),
				other_media->index);
		codec_store_populate(&other_media->codecs_send, &sp->codecs, flags->codec_set);
		codec_store_strip(&other_media->codecs_send, &flags->codec_strip, flags->codec_except);
		codec_store_offer(&other_media->codecs_send, &flags->codec_offer, &sp->codecs);
		if (!other_media->codecs_send.strip_full)
			codec_store_offer(&other_media->codecs_send, &flags->codec_transcode, &sp->codecs);
		codec_store_accept(&other_media->codecs_send, &flags->codec_accept, NULL);
		codec_store_accept(&other_media->codecs_send, &flags->codec_consume, &sp->codecs);
		codec_store_track(&other_media->codecs_send, &flags->codec_mask);
		...
		if (update_answerer) {
			...
			ilogs(codec, LOG_DEBUG, "Updating receiver side codecs for answerer " STR_FORMAT " #%u",
					...
					media->index);
			codec_store_populate(&media->codecs_recv, &sp->codecs, NULL);
		}
		codec_store_strip(&media->codecs_recv, &flags->codec_strip, flags->codec_except);
		codec_store_strip(&media->codecs_recv, &flags->codec_consume, flags->codec_except);
		codec_store_strip(&media->codecs_recv, &flags->codec_mask, flags->codec_except);
		codec_store_offer(&media->codecs_recv, &flags->codec_offer, &sp->codecs);
		codec_store_transcode(&media->codecs_recv, &flags->codec_transcode, &sp->codecs);
		codec_store_synthesise(&media->codecs_recv, &other_media->codecs_recv);

		if (update_answerer) {
			// do the same for sender side side
			ilogs(codec, LOG_DEBUG, "Updating sender side codecs for answerer " STR_FORMAT " #%u",
					STR_FMT(&media->monologue->tag),
					media->index);
			codec_store_populate(&media->codecs_send, &sp->codecs, NULL);
		}
		codec_store_strip(&media->codecs_send, &flags->codec_strip, flags->codec_except);
		codec_store_strip(&media->codecs_send, &flags->codec_consume, flags->codec_except);
		codec_store_strip(&media->codecs_send, &flags->codec_mask, flags->codec_except);
		codec_store_offer(&media->codecs_send, &flags->codec_offer, &sp->codecs);
		codec_store_transcode(&media->codecs_send, &flags->codec_transcode, &sp->codecs);
		codec_store_synthesise(&media->codecs_send, &other_media->codecs_send);

		codec_tracker_finish(&media->codecs_recv);
		codec_tracker_finish(&other_media->codecs_recv);
	}
	else {
		// answer
		ilogs(codec, LOG_DEBUG, "Updating receiver side codecs for answerer " STR_FORMAT " #%u",
		...
		//codec_store_answer(&media->codecs_recv, &other_media->codecs_recv, flags);
		...
		ilogs(codec, LOG_DEBUG, "Codec answer for " STR_FORMAT " #%u",
				STR_FMT(&other_media->monologue->tag),
				other_media->index);
		codec_store_answer(&media->codecs_send, &other_media->codecs_send, flags);
		codec_store_answer(&media->codecs_recv, &other_media->codecs_recv, flags);

		// XXX ??? codec_tracker_finish(&media->codecs_recv);
		// XXX ??? codec_tracker_finish(&other_media->codecs_recv);
	}
}
...
void call_media_free(struct call_media **mdp) {
	...
	codec_store_cleanup(&md->codecs_recv);
	codec_store_cleanup(&md->codecs_send);
